...
here::i_am("R/bayou_analyses/extant/bayou_extant_k15.R")
...
tl.bayou.extant <- setNames(imput.data$anc_recon[1:Ntip(slater.tree.pruned)], rownames(imput.data$anc_recon)[1:Ntip(slater.tree.pruned)])
tl.bayou <- tl.bayou.extant[names(tl.bayou.extant) %in% extant.tree$tip.label]
...
priorOU.extant.k15 <- make.prior(extant.tree, 
                      ...
                      param = list(dalpha = list(scale = 0.01), dsig2 = list(scale = 0.01),
                                 dk = list(lambda = 15, kmax = 1000), dsb = list(bmax = Inf, prob = extant.tree$edge.length), 
                                 dtheta = list(mean = 3.3578, sd = 7.887322e-02)),
                      plot.prior = FALSE
                      )
## startpars.extant.k15 <- priorSim(priorOU.extant.k15, extant.tree, plot = FALSE)$pars[[1]]

## Setting up and running MCMC - Full tree

mcmcOU.extant.k15 <- bayou.makeMCMC(extant.tree, tl.bayou, SE = se.bayou, prior = priorOU.extant.k15, file.dir = here::here("output/bayou/extant/bayou_extant_k15_fixedSE"), outname = "modelOU_imput_r001", plot.freq = NULL, samp = 1000) # Set up the MCMC

## mcmcOU.extant.k15$run(1000000) # Run the MCMC
