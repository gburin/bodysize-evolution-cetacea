...
here::i_am("R/bayou_analyses/extant/bayou_extant_k5.R")
...
priorOU.extant.k5 <- make.prior(extant.tree, 
                      ...
                      param = list(dalpha = list(scale = 0.01), dsig2 = list(scale = 0.01),
                                 dk = list(lambda = 5, kmax = 1000), dsb = list(bmax = Inf, prob = extant.tree$edge.length), 
                                 dtheta = list(mean = 3.3578, sd = 7.887322e-02)),
                      plot.prior = FALSE
                      )
## startpars.extant.k5 <- priorSim(priorOU.extant.k5, extant.tree, plot = FALSE)$pars[[1]]

## Setting up and running MCMC - Full tree

mcmcOU.extant.k5 <- bayou.makeMCMC(extant.tree, tl.bayou, SE = se.bayou, prior = priorOU.extant.k5, file.dir = here::here("output/bayou/extant/bayou_extant_k5_fixedSE"), outname = "modelOU_imput_r001", plot.freq = NULL, samp = 1000) # Set up the MCMC

## mcmcOU.extant.k5$run(1000000) # Run the MCMC
