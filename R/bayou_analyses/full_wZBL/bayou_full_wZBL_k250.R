...
here::i_am("R/bayou_analyses/full_wZBL/bayou_full_wZBL_k250.R")
...
tl.bayou <- setNames(imput.data$anc_recon[1:Ntip(slater.tree.imput)], rownames(imput.data$anc_recon)[1:Ntip(slater.tree.imput)])
...
priorOU.full.wZBL.k250 <- make.prior(slater.tree.imput, 
...
mcmcOU.full.wZBL.k250 <- bayou.makeMCMC(slater.tree.imput, tl.bayou, SE = se.bayou, prior = priorOU.full.wZBL.k250, file.dir = here::here("output/bayou/full_wZBL/bayou_full_wZBL_k250_fixedSE"), outname = "modelOU_imput_r001", plot.freq = NULL, samp = 1000) # Set up the MCMC

## mcmcOU.full.wZBL.k250$run(1000000) # Run the MCMC
