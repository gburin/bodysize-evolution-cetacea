...
here::i_am("R/bayou_analyses/noimput/bayou_noimput_k250.R")
...
tl.per.species <- aggregate(logdata.clean$tl, by = list(logdata.clean$species), FUN = mean, na.rm = TRUE)

tl.bayou <- setNames(tl.per.species[,2], tl.per.species[,1])
tl.bayou <- tl.bayou[!is.nan(tl.bayou)]
...
slater.tree.imput <- drop.tip(slater.tree.imput, slater.tree.imput$tip.label[is.na(match(slater.tree.imput$tip.label, names(tl.bayou)))])

## Setting priors - Noimput tree

priorOU.noimput.k250 <- make.prior(slater.tree.imput, 
...
## startpars.noimput.k250 <- priorSim(priorOU.noimput.k250, slater.tree.imput, plot = FALSE)$pars[[1]]


## Setting up and running MCMC - Noimput tree

mcmcOU.noimput.k250 <- bayou.makeMCMC(slater.tree.imput, tl.bayou, SE = se.bayou, prior = priorOU.noimput.k250, file.dir = here::here("output/bayou/noimput/bayou_noimput_k250_fixedSE"), outname = "modelOU_imput_r001", plot.freq = NULL, samp = 1000) # Set up the MCMC

## mcmcOU.noimput.k250$run(1000000) # Run the MCMC
