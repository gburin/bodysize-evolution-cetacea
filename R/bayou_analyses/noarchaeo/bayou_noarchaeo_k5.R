...
here::i_am("R/bayou_analyses/noarchaeo/bayou_noarchaeo_k5.R")
...
priorOU.noarchaeo.k5 <- make.prior(slater.tree.noarchaeo, 
                      ...
                      param = list(dalpha = list(scale = 0.01), dsig2 = list(scale = 0.01),
                                 dk = list(lambda = 5, kmax = 1000), dsb = list(bmax = Inf, prob = slater.tree.noarchaeo$edge.length), 
                                 dtheta = list(mean = mean(tl.bayou), sd = 1.5 * sd(tl.bayou))),
                      plot.prior = FALSE
                      )
## startpars.noarchaeo.k5 <- priorSim(priorOU.noarchaeo.k5, slater.tree.noarchaeo, plot = FALSE)$pars[[1]]


## Setting up and running MCMC - Full tree

mcmcOU.noarchaeo.k5 <- bayou.makeMCMC(slater.tree.noarchaeo, tl.bayou, SE = se.bayou, prior = priorOU.noarchaeo.k5, file.dir = here::here("output/bayou/noarchaeo/bayou_noarchaeo_k5_fixedSE"), outname = "modelOU_imput_r001", plot.freq = NULL, samp = 1000) # Set up the MCMC

## mcmcOU.noarchaeo.k5$run(1000000) # Run the MCMC
