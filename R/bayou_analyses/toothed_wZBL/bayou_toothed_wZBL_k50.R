...
here::i_am("R/bayou_analyses/toothed_wZBL/bayou_toothed_wZBL_k50.R")
...
priorOU.toothed.wZBL.k50 <- make.prior(toothed.tree, 
                      ...
                      param = list(dalpha = list(scale = 0.01), dsig2 = list(scale = 0.01),
                                 dk = list(lambda = 50, kmax = 1000), dsb = list(bmax = Inf, prob = toothed.tree$edge.length), 
                                 dtheta = list(mean = mean(tl.bayou), sd = 1.5 * sd(tl.bayou))),
                      plot.prior = FALSE
                      )
## startpars.toothed.k50 <- priorSim(priorOU.toothed.k50, toothed.tree, plot = FALSE)$pars[[1]]


## Setting up and running MCMC - Full tree

mcmcOU.toothed.wZBL.k50 <- bayou.makeMCMC(toothed.tree, tl.bayou, SE = se.bayou, prior = priorOU.toothed.wZBL.k50, file.dir = here::here("output/bayou/toothed_wZBL/bayou_toothed_wZBL_k50_fixedSE"), outname = "modelOU_imput_r001", plot.freq = NULL, samp = 1000) # Set up the MCMC

## mcmcOU.toothed.wZBL.k50$run(1000000) # Run the MCMC
