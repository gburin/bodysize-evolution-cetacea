...
here::i_am("R/bayou_analyses/toothed_wZBL/bayou_toothed_wZBL_k15.R")
...
logdata.clean <- logdata[logdata$species %in% toothed.tree$tip.label, ]
...
tl.bayou.toothed <- setNames(imput.data$anc_recon[1:Ntip(slater.tree.pruned)], rownames(imput.data$anc_recon)[1:Ntip(slater.tree.pruned)])
tl.bayou <- tl.bayou.toothed[names(tl.bayou.toothed) %in% toothed.tree$tip.label]
...
priorOU.toothed.wZBL.k15 <- make.prior(toothed.tree, 
                      ...
                      param = list(dalpha = list(scale = 0.01), dsig2 = list(scale = 0.01),
                                 dk = list(lambda = 15, kmax = 1000), dsb = list(bmax = Inf, prob = toothed.tree$edge.length), 
                                 dtheta = list(mean = mean(tl.bayou), sd = 1.5 * sd(tl.bayou))),
                      plot.prior = FALSE
                      )
## startpars.toothed.k15 <- priorSim(priorOU.toothed.k15, toothed.tree, plot = FALSE)$pars[[1]]


## Setting up and running MCMC - Full tree

mcmcOU.toothed.wZBL.k15 <- bayou.makeMCMC(toothed.tree, tl.bayou, SE = se.bayou, prior = priorOU.toothed.wZBL.k15, file.dir = here::here("output/bayou/toothed_wZBL/bayou_toothed_wZBL_k15_fixedSE"), outname = "modelOU_imput_r001", plot.freq = NULL, samp = 1000) # Set up the MCMC

## mcmcOU.toothed.wZBL.k15$run(1000000) # Run the MCMC
