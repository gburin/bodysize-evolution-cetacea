...
here::i_am("R/bayou_analyses/noextant/bayou_noextant_k500.R")
...
slater.tree.imput <- drop.tip(slater.tree.pruned, drop.fossil(slater.tree.pruned)$tip.label)
slater.tree.imput <- paleotree::dropZLB(slater.tree.imput)
...
tl.bayou <- setNames(imput.data$anc_recon[match(slater.tree.imput$tip.label, rownames(imput.data$anc_recon)), "tl"], rownames(imput.data$anc_recon)[match(slater.tree.imput$tip.label, rownames(imput.data$anc_recon))])

## Calculating standard error for species with multiple measurements - including species that are not in the phylogeny in this case
...
priorOU.noextant.k500 <- make.prior(slater.tree.imput, 
                      ...
                      param = list(dalpha = list(scale = 0.01), dsig2 = list(scale = 0.01),
                                 dk = list(lambda = 500, kmax = 1000), dsb = list(bmax = Inf, prob = slater.tree.imput$edge.length), 
                                 dtheta = list(mean = mean(tl.bayou), sd = 1.5 * sd(tl.bayou))),
                      plot.prior = FALSE
                      )
#startpars.full.k500 <- priorSim(priorOU.full.k500, slater.tree.imput, plot = FALSE)$pars[[1]]


## Setting up and running MCMC - Full tree

mcmcOU.noextant.k500 <- bayou.makeMCMC(slater.tree.imput, tl.bayou, SE = se.bayou, prior = priorOU.noextant.k500, file.dir = here::here("output/bayou/noextant/bayou_noextant_k500_fixedSE"), outname = "modelOU_imput_r001", plot.freq = NULL, samp = 1000) # Set up the MCMC

## mcmcOU.noextant.k500$run(1000000) # Run the MCMC
