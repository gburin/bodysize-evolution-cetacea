...
here::i_am("R/bayou_analyses/noextant/bayou_noextant_k5.R")
...
priorOU.noextant.k5 <- make.prior(slater.tree.imput, 
                      ...
                      param = list(dalpha = list(scale = 0.01), dsig2 = list(scale = 0.01),
                                 dk = list(lambda = 5, kmax = 1000), dsb = list(bmax = Inf, prob = slater.tree.imput$edge.length), 
                                 dtheta = list(mean = mean(tl.bayou), sd = 1.5 * sd(tl.bayou))),
                      plot.prior = FALSE
                      )
#startpars.full.k5 <- priorSim(priorOU.full.k5, slater.tree.imput, plot = FALSE)$pars[[1]]


## Setting up and running MCMC - Full tree

mcmcOU.noextant.k5 <- bayou.makeMCMC(slater.tree.imput, tl.bayou, SE = se.bayou, prior = priorOU.noextant.k5, file.dir = here::here("output/bayou/noextant/bayou_noextant_k5_fixedSE"), outname = "modelOU_imput_r001", plot.freq = NULL, samp = 1000) # Set up the MCMC

## mcmcOU.noextant.k5$run(1000000) # Run the MCMC
