...
here::i_am("R/bayou_analyses/baleen_wZBL/bayou_baleen_wZBL_k15.R")
...
priorOU.baleen.wZBL.k15 <- make.prior(baleen.tree, 
                      ...
                      param = list(dalpha = list(scale = 0.01), dsig2 = list(scale = 0.01),
                                 dk = list(lambda = 15, kmax = 1000), dsb = list(bmax = Inf, prob = baleen.tree$edge.length), 
                                 dtheta = list(mean = mean(tl.bayou), sd = 1.5 * sd(tl.bayou))),
                      plot.prior = FALSE
                      )
## startpars.baleen.k15 <- priorSim(priorOU.baleen.k15, baleen.tree, plot = FALSE)$pars[[1]]

## Setting up and running MCMC - Full tree

mcmcOU.baleen.wZBL.k15 <- bayou.makeMCMC(baleen.tree, tl.bayou, SE = se.bayou, prior = priorOU.baleen.wZBL.k15, file.dir = here::here("output/bayou/baleen_wZBL/bayou_baleen_wZBL_k15_fixedSE"), outname = "modelOU_imput_r001", plot.freq = NULL, samp = 1000) # Set up the MCMC

## mcmcOU.baleen.wZBL.k15$run(1000000) # Run the MCMC
