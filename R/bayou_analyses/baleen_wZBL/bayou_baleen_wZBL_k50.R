...
here::i_am("R/bayou_analyses/baleen_wZBL/bayou_baleen_wZBL_k50.R")
...
baleen.tree <- extract.clade(slater.tree.imput, getMRCA(slater.tree.imput, c("Mystacodon_selenensis", "Balaenoptera_brydei")))
...
tl.bayou.baleen <- setNames(imput.data$anc_recon[1:Ntip(slater.tree.pruned)], rownames(imput.data$anc_recon)[1:Ntip(slater.tree.pruned)])
tl.bayou <- tl.bayou.baleen[names(tl.bayou.baleen) %in% baleen.tree$tip.label]
...
priorOU.baleen.wZBL.k50 <- make.prior(baleen.tree, 
                      ...
                      param = list(dalpha = list(scale = 0.01), dsig2 = list(scale = 0.01),
                                 dk = list(lambda = 50, kmax = 1000), dsb = list(bmax = Inf, prob = baleen.tree$edge.length), 
                                 dtheta = list(mean = mean(tl.bayou), sd = 1.5 * sd(tl.bayou))),
                      plot.prior = FALSE
                      )
## startpars.baleen.k50 <- priorSim(priorOU.baleen.k50, baleen.tree, plot = FALSE)$pars[[1]]

## Setting up and running MCMC - Full tree

mcmcOU.baleen.wZBL.k50 <- bayou.makeMCMC(baleen.tree, tl.bayou, SE = se.bayou, prior = priorOU.baleen.wZBL.k50, file.dir = here::here("output/bayou/baleen_wZBL/bayou_baleen_wZBL_k50_fixedSE"), outname = "modelOU_imput_r001", plot.freq = NULL, samp = 1000) # Set up the MCMC

## mcmcOU.baleen.wZBL.k50$run(1000000) # Run the MCMC
