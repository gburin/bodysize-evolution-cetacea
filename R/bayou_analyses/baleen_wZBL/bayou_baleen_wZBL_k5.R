...
here::i_am("R/bayou_analyses/baleen_wZBL/bayou_baleen_wZBL_k5.R")
...
priorOU.baleen.wZBL.k5 <- make.prior(baleen.tree, 
                      ...
                      param = list(dalpha = list(scale = 0.01), dsig2 = list(scale = 0.01),
                                 dk = list(lambda = 5, kmax = 1000), dsb = list(bmax = Inf, prob = baleen.tree$edge.length), 
                                 dtheta = list(mean = mean(tl.bayou), sd = 1.5 * sd(tl.bayou))),
                      plot.prior = FALSE
                      )
## startpars.baleen.k5 <- priorSim(priorOU.baleen.k5, baleen.tree, plot = FALSE)$pars[[1]]

## Setting up and running MCMC - Full tree

mcmcOU.baleen.wZBL.k5 <- bayou.makeMCMC(baleen.tree, tl.bayou, SE = se.bayou, prior = priorOU.baleen.wZBL.k5, file.dir = here::here("output/bayou/baleen_wZBL/bayou_baleen_wZBL_k5_fixedSE"), outname = "modelOU_imput_r001", plot.freq = NULL, samp = 1000) # Set up the MCMC

## mcmcOU.baleen.wZBL.k5$run(1000000) # Run the MCMC
