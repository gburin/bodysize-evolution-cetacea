...
here::i_am("R/bayou_analyses/noarchaeo_wZBL/bayou_noarchaeo_wZBL_k500.R")
...
priorOU.noarchaeo.wZBL.k500 <- make.prior(slater.tree.noarchaeo, 
                      ...
                      param = list(dalpha = list(scale = 0.01), dsig2 = list(scale = 0.01),
                                 dk = list(lambda = 500, kmax = 1000), dsb = list(bmax = Inf, prob = slater.tree.noarchaeo$edge.length), 
                                 dtheta = list(mean = mean(tl.bayou), sd = 1.5 * sd(tl.bayou))),
                      plot.prior = FALSE
                      )
## startpars.noarchaeo.k5000 <- priorSim(priorOU.noarchaeo.k5000, slater.tree.noarchaeo, plot = FALSE)$pars[[1]]


## Setting up and running MCMC - Full tree

mcmcOU.noarchaeo.wZBL.k500 <- bayou.makeMCMC(slater.tree.noarchaeo, tl.bayou, SE = se.bayou, prior = priorOU.noarchaeo.wZBL.k500, file.dir = here::here("output/bayou/noarchaeo_wZBL/bayou_noarchaeo_wZBL_k500_fixedSE"), outname = "modelOU_imput_r001", plot.freq = NULL, samp = 1000) # Set up the MCMC

## mcmcOU.noarchaeo.wZBL.k500$run(1000000) # Run the MCMC
